...
12. **Neither direction of "owed" ever enters a spending total.** Gotcha 11 covers money owed *to* you; `personal_debts` and `debt_payments` follow the same rule. Repayments were briefly subtracted from Remaining (2026-08-24) and it was wrong in practice: because a month charges all its fixed costs on day one but earns its salary across the month, a new month opened thousands in the red before a single payday landed, and a big repayment made it far worse. The user saw −$6,887.50 on 1 September and asked for it out. Payments now feed only the "Paid back this month" tile — a record, not a deduction — and the trend chart leaves them out too, because a chart disagreeing with the headline figure is worse than one that omits something. What's left on a debt is still `amount − sum(payments)`, derived and never stored, so no balance can drift from the payments meant to explain it. Keep the two tables apart regardless: summing them would net off two unrelated things.
